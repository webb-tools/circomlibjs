...
export function createCode(nInputs) {
    ...
    const strLabel0 = "after_pix";
    C._pushLabel(strLabel0);
    C.push(0);
    C.mstore();
...
//state = state.map((a, i) => F.add(a, C[(r + 1)* t +i]));
    ...
    for(let r = 0; r < nRoundsF/2-1; r++) {
        ...
        //     state.reduce((acc, a, j) => F.add(acc, F.mul(M[j][i], a)), F.zero)
        // );
        const strLabelP = "aferMix"+nRoundsF/2+nRoundsP+r;
        C._pushLabel(strLabelP);
        C.push(0);
        C.mstore();
    ...
    const strLabel1 = "aferMix_final";
    C._pushLabel(strLabel1);
    C.push(0);
    C.mstore();
    C.jmp("mix"); // multiply by M
    C.label(strLabel1);
